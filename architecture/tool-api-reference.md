...
| `styleId` | 固定空字符串 | 固定空字符串 | 动态 | NewStyle 返回的 `id/styleId`，始终作为字符串 |
...
对接方调用 `get_print_info` 时，Token 和 reportName 都在 HTTP 请求头中传入：
...
| 参数 | 类型 | 必填 | 动态/固定 | 说明 |
|---|---|---|---|---|
| `report_type` | integer | 是 | 动态 | 当前业务的报表类型，例如 `1` |
| `style_name` | string | 是 | 动态 | 用户指定的新模板名称 |
...
| 参数 | 类型 | 必填 | 动态/固定 | 说明 |
|---|---|---|---|---|
| `report_type` | integer | 是 | 动态 | 使用 NewStyle 返回的 `reportType` |
| `style_name` | string | 是 | 动态 | 使用 NewStyle 返回的 `styleName` |
| `style_id` | string | 是 | 动态 | 使用 NewStyle 返回的 `styleId` |
| `style_content` | object | 是 | 动态 | 本轮生成或编辑后的完整原生模板 JSON |
...
| `STYLE_INVALID` | X-Report-Name 头缺失或为空 |
| `CAPABILITY_FORBIDDEN` | 缺少 read/write scope |
| `YUNPRINT_REQUEST_FAILED` | HTTP、网络或业务失败 |
| `YUNPRINT_RESPONSE_INVALID` | 响应信封、类型或新增模板 ID 无效 |
| `STYLE_INVALID` | 名称、模板 ID 或 reportType 无效 |
| `STYLE_CONTENT_INVALID` | 模板 JSON 为空或无法序列化 |

GetPrintInfo 可对网络错误和 5xx 重试。NewStyle 与 SaveStyle 不自动重试。
